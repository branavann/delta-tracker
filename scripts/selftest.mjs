...
import { historyRow } from './snapshot.mjs';
...
ok('turnover = volume over market cap',
  near(d.turnover24hPct, (d.volume24hUsd / d.supply.marketCapUsd) * 100));
ok('FDV = total supply x price', near(d.supply.fdvUsd, d.supply.total * d.priceUsd));
ok('market cap = circulating x price', near(d.supply.marketCapUsd, d.supply.circulating * d.priceUsd));
